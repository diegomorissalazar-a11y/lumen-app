...
function updateHistoriaFieldsVisibility(){const panel=document.getElementById('historia-fields');if(!panel)return;const active=!!document.querySelector('#generos-chips .genero-chip[data-g="Historia"].active');panel.style.display=active?'block':'none';if(active){const current=selectedHistoricalLineIds('hist-line-flags');renderHistoricalLineFlags('hist-line-flags',current);}}
function getHistoriaFromForm(){const lineaIds=selectedHistoricalLineIds('hist-line-flags'),lineas=historicalLineNamesFromIds(lineaIds),ri=document.getElementById('hist-fecha-inicio')?.value,rf=document.getElementById('hist-fecha-fin')?.value,inicio=ri===''||ri==null?null:Number(ri),fin=rf===''||rf==null?null:Number(rf),lineKey=lineaIds.slice().sort().join('+')||'sin_linea';return{schema:'lumen_historia_v5',lineaIds,lineas,lineaPrincipalId:lineaIds[0]||'',lineaPrincipal:lineas[0]||'',lineasRelacionadasIds:lineaIds.slice(1),lineasRelacionadas:lineas.slice(1),fechaInicio:inicio,fechaFin:fin,periodos:(inicio!==null||fin!==null)?[{nombre:'',tema:'',inicio:inicio??fin,fin:fin??inicio,precision:'aproximada',cobertura:'periodo',periodoId:canonicalEntityId('histperiod',`${lineKey}|${inicio??fin}_${fin??inicio}`)}]:[]}}
function resetHistoriaFields(){['hist-fecha-inicio','hist-fecha-fin'].forEach(id=>{const el=document.getElementById(id);if(el)el.value=''});renderHistoricalLineFlags('hist-line-flags',[])}
function setHistoriaFields(historia){const h=historia||{},b=historyBounds(h),set=(id,v)=>{const el=document.getElementById(id);if(el)el.value=v??''},tmp={type:'libro',historia:{...h}};ensureHistoriaCanonicalRefs(tmp);set('hist-fecha-inicio',b.inicio);set('hist-fecha-fin',b.fin);updateHistoriaFieldsVisibility();renderHistoricalLineFlags('hist-line-flags',historyLineIdsForEntry(tmp))}
