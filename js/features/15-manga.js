...
(function(){
  ...
  window.finishManga=function(){const e=db.entries.find(x=>x.id===document.getElementById('manga-progress-id').value);if(!e)return;const now=new Date();e.capActual=parseInt(document.getElementById('manga-progress-current').value)||e.capActual||0;e.capTotal=parseInt(document.getElementById('manga-progress-total').value)||e.capTotal||0;e.estado_publicacion=document.getElementById('manga-progress-publication').value;e.estado='leido';e.mes=MESES[now.getMonth()];e.anio=now.getFullYear();e._updatedAt=Date.now();if(typeof applyLocalCoverPolicy==='function')applyLocalCoverPolicy(e);saveDB();closeModal('modal-manga-progress');showToast('✓ Manga terminado');renderHome();if(currentScreen==='library')renderLibrary();};
