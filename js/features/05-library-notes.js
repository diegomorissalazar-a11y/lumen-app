...
function entryCardHTML(e) {
  ...
  const generosHtml = e.type === 'libro' ? (() => {
    ...
    if(gs.includes('Historia')) historyLineNamesForEntry(e).forEach(x=>secondary.push(x));
    const collection=e.collectionMetadata||e.cuentos;if((gs.includes('Cuento')||gs.includes('Poesía'))&&collection?.esRecopilacion)secondary.push(collection.tipoRecopilacion==='antologia'?'Antología':'Recopilación');
...
function openHistoriaQuick(entryId){const e=(db.entries||[]).find(x=>x.id===entryId);if(!e||e.type!=='libro'||!(e.generos||[]).includes('Historia'))return;const launch=()=>{ensureHistoriaCanonicalRefs(e);const h=e.historia||{},b=historyBounds(h);document.getElementById('histq-entry-id').value=e.id;document.getElementById('histq-book').textContent=e.titulo||'';document.getElementById('histq-inicio').value=b.inicio??'';document.getElementById('histq-fin').value=b.fin??'';renderHistoricalLineFlags('histq-line-flags',historyLineIdsForEntry(e));openModal('modal-historia-quick')};const d=document.getElementById('modal-detail');if(d?.classList.contains('open')){closeModal('modal-detail');setTimeout(launch,140)}else launch()}
function saveHistoriaQuick(){
  return lumenSafeAction("Guardar Historia", () => {const id=document.getElementById('histq-entry-id').value,e=(db.entries||[]).find(x=>x.id===id);if(!e)return;const lineaIds=selectedHistoricalLineIds('histq-line-flags'),lineas=historicalLineNamesFromIds(lineaIds),ri=document.getElementById('histq-inicio').value,rf=document.getElementById('histq-fin').value,inicio=ri===''?null:Number(ri),fin=rf===''?null:Number(rf),lineKey=lineaIds.slice().sort().join('+')||'sin_linea';e.historia={...(e.historia||{}),schema:'lumen_historia_v5',lineaIds,lineas,lineaPrincipalId:lineaIds[0]||'',lineaPrincipal:lineas[0]||'',lineasRelacionadasIds:lineaIds.slice(1),lineasRelacionadas:lineas.slice(1),fechaInicio:inicio,fechaFin:fin,periodos:(inicio!==null||fin!==null)?[{nombre:'',tema:'',inicio:inicio??fin,fin:fin??inicio,precision:'aproximada',cobertura:'periodo',periodoId:canonicalEntityId('histperiod',`${lineKey}|${inicio??fin}_${fin??inicio}`)}]:[]};ensureHistoriaCanonicalRefs(e);e._updatedAt=Date.now();if(typeof invalidateRecommendations==='function')invalidateRecommendations('historia modificada');const ok=saveDB();if(ok!==false){closeModal('modal-historia-quick');showToast('✓ Historia actualizada');if(currentScreen==='library')renderLibrary();if(currentScreen==='mapas')renderMapaHistoria()}
  });
}
...
function showDetail(id) {
  ...
  if (e.type === 'libro') {
    ...
    if (e.historia && (e.generos||[]).includes('Historia')) {
      ...
      const historyLines=historyLineNamesForEntry(e);
      if(historyLines.length) html += row('Líneas históricas', historyLines.join(', '));
...
function editEntry(id) {
  ...
  setTimeout(() => {
    ...
    if (e.type === 'libro') {
      ...
      if (typeof setLiteraryMetadataFields === 'function') setLiteraryMetadataFields(e);
    } else if (e.type === 'pelicula') {
